    ...
    while running:
        ...
        if game_state == GAME_STATE_MENU:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    running = False
                if event.type == pygame.MOUSEBUTTONDOWN: # Click to restart
                    # Reset game
                    main() # This is a simple way to restart
                    return # Exit the current (dead) game loop

            # Draw the "YOU DIED" screen (your existing code)
            text = game_over_font.render("YOU DIED", True, (150, 0, 0))
            # ... (rest of your game over drawing)

            # Add a "Click to restart" prompt
            prompt_text = ui_font.render("You Died!! Click anywhere to restart", True, WHITE)
